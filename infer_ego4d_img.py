...
def main():
    ...
    parser.add_argument("--data-root", required=True,
                        help="Root dir containing origin/blur/deblur subfolders (e.g. wbLee/data/Ego4D/image)")
    ...
    total = len(CONDITIONS) * len(CLIP_UIDS)
    idx = 0
    skipped = []
    ...
    for condition in CONDITIONS:
        for clip_uid in CLIP_UIDS:
            idx += 1
            image_dir = os.path.join(args.data_root, condition, clip_uid)
            if not os.path.isdir(image_dir):
                print(f"[{idx:2d}/{total}] SKIP (not found): {condition}/{clip_uid}")
                skipped.append(f"{condition}/{clip_uid}")
                continue

            print(f"[{idx:2d}/{total}] Processing: {condition}/{clip_uid}")

            try:
                predictions = run_inference(image_dir, model, args.image_resolution)

                out_dir = os.path.join(args.output_dir, condition, clip_uid)
    ...
    print(f"\nDone. Skipped {len(skipped)}/{total}:")
    for s in skipped:
        print(f"  {s}")
