"""
Reconstruct rotation_high_011 with two frame sets:

1. all_60: frames 1..60 extracted at 6 fps from the 0s..10s clip
2. frames_025_060: frames 25..60 only, reconstructed by running VGGT-Omega
   on those 36 frames only
3. all_60/frames_025_060_from_all_60.ply: run VGGT-Omega on all 60 frames,
   then visualize only the point clouds from frames 25..60

Outputs:
  outputs/renders/vggt-omega/ego4d/high011_subsets/<clip_name>/all_60/
  outputs/renders/vggt-omega/ego4d/high011_subsets/<clip_name>/frames_025_060/
    scene.ply
    predictions_pose.npz
    camera_trajectory.png
    metadata.json
  outputs/renders/vggt-omega/ego4d/high011_subsets/<clip_name>/all_60/
    frames_025_060_from_all_60.ply
"""
...
CLIP_NAME = "rotation_high_011__1b3c0015-a687-4237-abb5-cde1bfd1e85f"
DEFAULT_VIDEO = os.path.join(PROJECT_ROOT, "data/ego4d/v2/label/rotation_high", f"{CLIP_NAME}.mp4")
DEFAULT_OUTPUT_DIR = os.path.join(
    PROJECT_ROOT,
    "outputs/renders/vggt-omega/ego4d/high011_subsets",
    CLIP_NAME,
)
DEFAULT_FPS = 6.0
...
def save_selected_frames_from_all_prediction(
    predictions: dict,
    output_path: str,
    start_frame: int,
    end_frame: int,
    ...
    indices = list(range(start_frame - 1, end_frame))
    if min(indices) < 0 or max(indices) >= len(predictions["extrinsic"]):
        raise ValueError(
            f"Cannot select frames {start_frame}..{end_frame} from "
            f"{len(predictions['extrinsic'])} predicted frames"
        )

    selected = slice_predictions(predictions, indices)
    from visual_util import predictions_to_ply

    pc = predictions_to_ply(
        selected,
        conf_thres=reconstruction.CONF_THRES,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--subset-start-frame", type=int, default=25)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_root:
        ...
        subset_dir = os.path.join(tmp_root, "frames_025_060")
        ...
        all_predictions = reconstruct_subset(
            reconstruction,
            model,
        ...
        if all_predictions is not None:
            save_selected_frames_from_all_prediction(
                all_predictions,
                output_path=os.path.join(
                    args.output_dir,
                    "all_60",
                    f"frames_{args.subset_start_frame:03d}_{args.subset_end_frame:03d}_from_all_60.ply",
                ),
                start_frame=args.subset_start_frame,
                end_frame=args.subset_end_frame,
        ...
        reconstruct_subset(
            ...
            out_dir=os.path.join(args.output_dir, f"frames_{args.subset_start_frame:03d}_{args.subset_end_frame:03d}"),
